...
namespace CollegeManagement.API.Web.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<ActionResult<FacultyListResponseWithPaginationVM>> GetAllFacultiesWithPagination([FromRoute] int pageNumber, int pageSize)
        {
            try
            {
                _logger.LogInformation("Started processing {namespace} AdminController", typeof(AdminController).Namespace);
                var result = await _mediator.Send<FacultyListResponseWithPaginationVM>(new GetAllFacultyListWithPagination(pageNumber, pageSize));
